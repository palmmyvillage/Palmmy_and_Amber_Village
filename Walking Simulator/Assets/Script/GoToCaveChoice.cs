...
public class GoToCaveChoice : MonoBehaviour {

	public GameObject ChoiceBox;
	public GameObject GoToCaveDialogueBox;
	public GameObject GoToCaveObject;
		...
		if (Status == true)
		{
			...
			if (!Input.GetButtonDown("Vertical"))
			{
				if (Input.GetMouseButtonDown(0))
				{
					...
					BGMogject.GetComponent<SoundFadeIn>().enabled = true;

					ChoiceBox.SetActive(false);
					GoToCaveDialogueBox.SetActive(false);
					GoToCaveObject.GetComponent<GoToCaveDialogue>().enabled = false;
				}
			}
		}
		else
